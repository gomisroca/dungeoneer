...
export default async function updateRaids() {
  ...
  for (const raid of raids) {
    ...
    // Get existing IDs
    const existingMinionIds = new Set(raid.minions.map(m => m.id));
    const existingEmoteIds = new Set(raid.emotes.map(e => e.id));
    const existingHairstyleIds = new Set(raid.hairstyles.map(h => h.id));
    const existingCardIds = new Set(raid.cards.map(c => c.id));
    const existingSpellIds = new Set(raid.spells.map(s => s.id));
    const existingOrchestrionIds = new Set(raid.orchestrions.map(o => o.id));
    const existingMountIds = new Set(raid.mounts.map(m => m.id));
    ...
    if (newMinions.length || newEmotes.length || newHairstyles.length || newCards.length || newSpells.length || newOrchestrions.length || newMounts.length) {
      ...
      console.log(`Updated raid ${raid.name} with:
      ...
      console.log(`No new connections found for raid ${raid.name}`);
    }
  }
}