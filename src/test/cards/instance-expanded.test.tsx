...
describe('InstanceCard', () => {
  ...
  it('renders correctly with no ownership and shows no indicator', () => {
    render(<InstanceCard instance={mockInstance} session={mockSession} />);

    // Main container present
    const container = screen.getByTestId('instance-card');
    ...
    expect(img).toHaveAttribute('src', mockInstance.image);
    expect(img).toHaveAttribute('alt', mockInstance.name);
  ...
  it('does not render image when instance.image is falsy', () => {
    const noImageInstance = { ...mockInstance, image: '' };
    render(<InstanceCard instance={noImageInstance} session={mockSession} />);
