import { z } from 'zod';
import { createTRPCRouter, protectedProcedure, publicProcedure } from '@/server/api/trpc';
import { TRPCError } from '@trpc/server';
import { type ExpandedOrchestrion } from 'types';

export const orchestrionsRouter = createTRPCRouter({
  getAll: publicProcedure
    .input(
      z.object({
        limit: z.number().min(1).max(100).nullish(),
        cursor: z.string().nullish(),
      })
    )
    .query(async ({ ctx, input }) => {
      const limit = input.limit ?? 10;
      const { cursor } = input;
      const orchestrions: ExpandedOrchestrion[] = await ctx.db.orchestrion.findMany({
        take: limit + 1,
        cursor: cursor ? { id: cursor } : undefined,
        orderBy: {
          id: 'desc',
        },
        include: {
          owners: true,
          sources: true,
        },
      });

      let nextCursor: typeof cursor | undefined = undefined;

      if (orchestrions.length > limit) {
        const nextOrchestrion = orchestrions.pop();
        nextCursor = nextOrchestrion!.id;
      }

      return {
        orchestrions,
        nextCursor,
      };
    }),

  addToUser: protectedProcedure
    .input(
      z.object({
        id: z.string(),
      })
    )
    .mutation(async ({ ctx, input }) => {
      const orchestrion = await ctx.db.orchestrion.findUnique({
        where: {
          id: input.id,
        },
        include: {
          owners: true,
        },
      });

      if (!orchestrion) {
        throw new TRPCError({ code: 'NOT_FOUND' });
      }

      const user = await ctx.db.user.findUnique({
        where: {
          id: ctx.session.user.id,
        },
        include: {
          orchestrions: true,
        },
      });

      if (!user) {
        throw new TRPCError({ code: 'NOT_FOUND' });
      }

      if (user.orchestrions.find((o) => o.id === orchestrion.id)) {
        throw new TRPCError({ code: 'CONFLICT' });
      }

      await ctx.db.user.update({
        where: {
          id: ctx.session.user.id,
        },
        data: {
          orchestrions: {
            connect: {
              id: orchestrion.id,
            },
          },
        },
      });

      await ctx.db.orchestrion.update({
        where: {
          id: orchestrion.id,
        },
        data: {
          owners: {
            connect: {
              id: ctx.session.user.id,
            },
          },
        },
      });
    }),

  removeFromUser: protectedProcedure
    .input(
      z.object({
        id: z.string(),
      })
    )
    .mutation(async ({ ctx, input }) => {
      const orchestrion = await ctx.db.orchestrion.findUnique({
        where: {
          id: input.id,
        },
        include: {
          owners: true,
        },
      });

      if (!orchestrion) {
        throw new TRPCError({ code: 'NOT_FOUND' });
      }

      const user = await ctx.db.user.findUnique({
        where: {
          id: ctx.session.user.id,
        },
        include: {
          orchestrions: true,
        },
      });

      if (!user) {
        throw new TRPCError({ code: 'NOT_FOUND' });
      }

      if (!user.orchestrions.find((o) => o.id === orchestrion.id)) {
        throw new TRPCError({ code: 'NOT_FOUND' });
      }

      await ctx.db.user.update({
        where: {
          id: ctx.session.user.id,
        },
        data: {
          orchestrions: {
            disconnect: {
              id: orchestrion.id,
            },
          },
        },
      });

      await ctx.db.orchestrion.update({
        where: {
          id: orchestrion.id,
        },
        data: {
          owners: {
            disconnect: {
              id: ctx.session.user.id,
            },
          },
        },
      });
    }),
});
